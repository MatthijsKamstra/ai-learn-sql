...
const DB_FILE = './demo.db'; // relative to script location
...
async function createDatabase() {
	...
	await db.exec('PRAGMA foreign_keys = ON;'); // 🔥 important
	...
	for (const i of order_items) {
		await insertItem.run(i);
	}
	await insertItem.finalize();

	await db.close();
	console.log('✅ Default database created: demo.db');
}
